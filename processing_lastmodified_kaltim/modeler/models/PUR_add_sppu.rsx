...
##PUR_overlap=output raster
##passfilenames

wd <- WDir
PUR_data <- PUR_rec_data
setwd (wd)
...
rec.map<-raster(rec_data)
rec_data<-raster(rec_data)
rec.map<-ratify(rec.map, filename='reconciled_map.grd',count=TRUE,overwrite=TRUE)
...
ref_attribute<-as.data.frame(levels(rec.map))
ref.number<-file.number+1
colnames(ref_attribute)<-c((paste("Var",ref.number,sep="")),"rec_reference")
PUR2<-rec.map # initial reference map
command1<-paste(command1,",rec.map",  sep="") # input command for stack input
command2<-paste( command2,",rec.map[]", sep="") # input command to overlay process
...
overlapping[is.na(overlapping)]<-0
...
colnames(sppu_ref)<-c('ID','overlapping')
PUR_overlap<-PUR2
levels(PUR_overlap)<-merge((levels(PUR_overlap)),sppu_ref,by="ID", all.x=T)
#modify att table
overlap_tab<-as.data.frame(levels(PUR_overlap))
...
PUR_overlap_summary<-as.data.frame(levels(PUR_overlap))

#produce map and lookup table result
write.dbf(PUR_overlap_summary, "PUR_overlap_summary")
writeRaster(PUR_overlap, filename="PUR_overlap_area", format="GTiff", overwrite=TRUE)
