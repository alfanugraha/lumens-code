...
test1<-as.data.frame(c("Nama daerah", "Periode", "Data tutupan lahan t1", "Data tutupan lahan t2", "Data unit perencanaan", "Tabel acuan penggunaan lahan", "Tabel acuan unit perencanaan"))
colnames(test1)[1]<-"Data"
test2<-as.data.frame(c(location,proj_period, landuse_1, landuse_2, zone_l,lookup_lc2, lookup_zo))
colnames(test2)[1]<-"Keterangan"
...
Largest.chg<- ggplot(data=chg_only_top, aes(x=reorder(CHG_CODE, -COUNT),y=COUNT, fill=CHG_CODE))+geom_bar(stat='identity',position='dodge')+
  ...
  ggtitle(paste("10 Perubahan Tutupan Lahan Dominan di", location, period1,"-",period2 )) +
...
#WRITE REPORT
title<-"\\b\\fs40 LUMENS-Pre QUES Project Report\\b0\\fs20"
sub_title<-"\\b\\fs32 ANALISA PERUBAHAN PENGGUNAAN LAHAN\\b0\\fs20"
...
chapter1<-"\\b\\fs28 DATA YANG DIGUNAKAN \\b0\\fs20"
chapter2<-"\\b\\fs28 HASIL ANALISA PADA TINGKAT BENTANG LAHAN \\b0\\fs20"
chapter3<-"\\b\\fs28 HASIL ANALISA PADA TINGKAT UNIT PERENCANAAN \\b0\\fs20"
...
addNewLine(rtffile)
addParagraph(rtffile, paste("\\b \\fs20 Sepuluh Perubahan Lahan Dominan di\\b0 \\fs20 ", area_name_rep, I_O_period_1_rep, "\\b \\fs20 - \\b0 \\fs20", I_O_period_2_rep, sep=" "))
