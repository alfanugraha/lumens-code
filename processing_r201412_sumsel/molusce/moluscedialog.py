...
from processing.molusce.neuralnetworkwidget import NeuralNetworkWidget
from processing.molusce.weightofevidencewidget import WeightOfEvidenceWidget
from processing.molusce.multicriteriaevaluationwidget import MultiCriteriaEvaluationWidget
...
if not scipyMissed:
  from processing.molusce.logisticregressionwidget import LogisticRegressionWidget
  
  

from processing.molusce.ui.ui_moluscedialogbase import Ui_Dialog
...
from processing.molusce.algorithms.models.crosstabs.manager import CrossTableManager
from processing.molusce.algorithms.models.area_analysis.manager import AreaAnalyst
from processing.molusce.algorithms.models.sampler.sampler import SamplerError
from processing.molusce.algorithms.models.simulator.sim import Simulator
from processing.molusce.algorithms.models.errorbudget.ebmodel import EBudget

from processing.molusce.molusceutils  import *
...
class MolusceDialog(QDialog, Ui_Dialog):
  def __init__(self, iface):
    ...
    self.cmbUnitsMatrix.currentIndexChanged.connect(self.__drawTransitionStat)
    self.btnCreateChangeMap.clicked.connect(self.createChangeMap)
    self.initShortcuts()
    
    # tab transition potential modeling
    ...
    self.chkRiskFunction.toggled.connect(self.__toggleLineEdit) #certancy function
    self.chkRiskValidation.toggled.connect(self.__toggleLineEdit) #validation map
    self.chkMonteCarlo.toggled.connect(self.__toggleLineEdit) #simulation result
    self.chkTransitionPotentials.toggled.connect(self.__toggleLineEdit) #prefix of transition potential maps
  ...
  def setInitialRaster(self):
    ...
    layer = getLayerById(self.initRasterId)
    try:
      self.inputs["initial"] = Raster(unicode(layer.source()), maskVals=getLayerMaskById(self.initRasterId))
  ...
  def setFinalRaster(self):
    ...
    try:
      self.inputs["final"] = Raster(unicode(getLayerById(self.finalRasterId).source()), getLayerMaskById(self.finalRasterId))
    ...
    for i in layerNames:
      ...
      try:
        if "factors" in self.inputs:
          self.inputs["factors"][layerId] = Raster(unicode(getLayerById(layerId).source()), getLayerMaskById(layerId))
        else:
          d = dict()
          d[layerId] = Raster(unicode(getLayerById(layerId).source()), getLayerMaskById(layerId))
  ...
  def updateStatisticsTableDone(self):
    ...
    #print
#     self.logMessage("\nini crosstab dari class updateStatisticsTableDone.getTransitionMatrix\n%s\n" % self.inputs["crosstab"].getTransitionMatrix())
#     self.logMessage("\nini crosstab dari class updateStatisticsTableDone.getCrosstable\n%s\n" % self.inputs["crosstab"].getCrosstable().getCrosstable())
    
    #ngubah nilai
#     self.inputs["crosstab"].getCrosstable().getCrosstable()[1,1] = 10000
    #self.inputs["crosstab"].getTransitionMatrix()[1,1] = 0.123456789012 #ga ngaruh
    
    #print lagi
#     self.logMessage("\nini crosstab untuk dapet nilai i,j \n%s\n" % self.inputs["crosstab"].getCrosstable().getCrosstable()[1,1])
#     self.logMessage("\nini crosstab dari class updateStatisticsTableDone.getTransitionMatrix\n%s\n" % self.inputs["crosstab"].getTransitionMatrix()[1,1])
#     self.logMessage("\nini crosstab dari class updateStatisticsTableDone.getCrosstable\n%s\n" % self.inputs["crosstab"].getCrosstable().getCrosstable())
  ...
  def createChangeMap(self):
    ...
    fileName = saveRasterDialog(self,
                                      self.settings,
  ...
  def changeMapDone(self, raster):
    ...
    layer = getLayerByName(QFileInfo(self.inputs["changeMapName"]).baseName())
  ...
  def startSimulation(self):
    ...
    if not "ha" in self.cmbUnitsMatrix.currentText():
      QMessageBox.warning(self,
                          self.tr("Missed transition matrix unit"),
                          self.tr("Please change transition matrix unit to hectare and try again")
                         )
      return
         
    #get value from Transition Matrix widget in fraction
    transition = self.inputs["crosstab"].getTransitionMatrix()
    dimensions = len(transition)
    
    #get value from Transition Matrix widget in hectare
    #x = []
    self.logMessage2("Updated crosstab from widget")
    for i in range(dimensions):
      for j in range(dimensions):
        z = long(self.tblTransMatrix.item(i,j).text())
        self.inputs["crosstab"].getCrosstable().getCrosstable()[i,j] = z
        #x.append(int(z))
      #self.logMessage2("%s" % x)
      #x = []
    #self.logMessage2("\nini crosstab dari class terupdate\n%s\n" % self.inputs["crosstab"].getCrosstable().getCrosstable())
    #self.logMessage2("\nini crosstab dari class getTransitionMatrix terupdate\n%s\n" % self.inputs["crosstab"].getTransitionMatrix())    
  ...
  def simulationDone(self):
    ...
    if self.chkRiskFunction.isChecked():
      if not self.leRiskFunctionPath.text() == "":
        ...
        layer = getLayerByName(QFileInfo(self.leRiskFunctionPath.text()).baseName())
    ...
    if self.chkMonteCarlo.isChecked():
      if not self.leMonteCarloPath.text() == "":
        ...
        if copySymbology(getLayerByName(self.leInitRasterName.text()), getLayerByName(QFileInfo(self.leMonteCarloPath.text()).baseName())):
          layer = getLayerByName(QFileInfo(self.leMonteCarloPath.text()).baseName())
  ...
  def createValidationMap(self):
    try:
      ...
      reference = Raster(layerSource, getLayerMaskBySource(layerSource))
    except ProviderError:
      QMessageBox.warning(self,
                          self.tr("Can't read file"),
                          self.tr("Can't read file: '%s'" % unicode(self.leReferenceMapPath.text()))
    ...
    try:
      ...
      simulated = Raster(layerSource, getLayerMaskBySource(layerSource))
    except ProviderError:
      QMessageBox.warning(self,
                          self.tr("Can't read file"),
                          self.tr("Can't read file: '%s'" % unicode(self.leSimulatedMapPath.text()))
  ...
  def validationMapDone(self, raster):
    ...
    layer = getLayerByName(QFileInfo(self.inputs["valMapName"]).baseName())
...
# ******************************************************************************
  ...
  #Alfa 2014-02-03
  def __populateUnitsMatrix(self):
    self.cmbUnitsMatrix.addItems([
                                    self.tr("frac"),
                                    self.tr("ha"),
                                  ])  
    ...
    for k, v in self.inputs["factors"].iteritems():
      ...
      for b in xrange(v.getBandsCount()):
        if v.getBandsCount()>1:
          name = u"%s (band %s)" % (getLayerById(k).name(), unicode(b + 1))
        else:
          name = u"%s" % (getLayerById(k).name())
        mapping[k][b] = labNo
        labNo = labNo + 1
        labels.append(name)
  ...
  def __drawTransitionStat(self):
    ...
    # transitional matrix
    displayUnitsMatrix = self.cmbUnitsMatrix.currentText()    
    
    hectare = self.inputs["crosstab"].getCrosstable().getCrosstable()
    
    transition = self.inputs["crosstab"].getTransitionMatrix()
    dimensions = len(transition)

    self.tblTransMatrix.clear()
    self.tblTransMatrix.setRowCount(dimensions + 1)
    self.tblTransMatrix.setColumnCount(dimensions + 1)

#     if displayUnitsMatrix == self.tr("ha"):
#       if dimensions != 0 :
#         self.__updateTransitionMatrix()
      
      #habis dari sini pokonya langsung update perubahan
#     else:
#       self.tblTransMatrix.setEnabled(False)
    ...
    if "singlebandpseudocolor" in layer.renderer().type().lower():
      ...
      lenlbl = len(labels)
      if lenlbl < dimensions:
        for j in xrange(lenlbl, dimensions):
          labels.append(unicode(j + 1))      
        labels.append(unicode("Total"))
      else:
        labels.append(unicode("Total"))        
    else:
    ...
    model = QtGui.QStandardItemModel(dimensions, dimensions)
    #self.tblTransMatrix.setEditTriggers(QtGui.QAbstractItemView.AllEditTriggers)
    self.tblTransMatrix.viewport().setAttribute(QtCore.Qt.WA_Hover)    
    
    self.delegate = SpinBoxDelegate()  
    self.tblTransMatrix.setItemDelegate(self.delegate)
    ...
    for row in xrange(0, dimensions):
      for col in xrange(0, dimensions):
        if displayUnitsMatrix == self.tr("ha"):
          ...
          it = transition[col, row]
          sumOfCol += it
      totalCol.append(unicode(sumOfCol))
      sumOfCol = 0
                   
    self.__addTotalColumn(dimensions, totalRow, totalCol)
    
    self.tblTransMatrix.resizeRowsToContents()
    self.tblTransMatrix.resizeColumnsToContents()

  def __addTotalColumn(self, col, valRow, valCol):
    dim = len(valRow)
    sumAll = 0
    for r in xrange(0, dim):
      item = QTableWidgetItem(unicode(valRow[r]))
      item.setFlags(Qt.ItemIsEnabled|Qt.ItemIsSelectable)
      self.tblTransMatrix.setItem(r, col, item)
      
      sumAll += float(valRow[r])
      
      item = QTableWidgetItem(unicode(valCol[r]))
      item.setFlags(Qt.ItemIsEnabled|Qt.ItemIsSelectable)
      self.tblTransMatrix.setItem(col, r, item)
    item = QTableWidgetItem(unicode(sumAll))
    item.setFlags(Qt.ItemIsEnabled|Qt.ItemIsSelectable)
    self.tblTransMatrix.setItem(col, col, item)
  ...
  def __selectSamplesOutput(self):
    ...
    fileName = saveVectorDialog(self,
                                      self.settings,
                                      self.tr("Save file"),
  ...
  def __selectSimulationOutput(self):
    ...
    if senderName == "btnSelectTransitionPrefix":
      dirname = openDirectoryDialog(self,
                                          self.settings,
  ...
  def __selectValidationMap(self):
    ...
    fileName = openRasterDialog(self,
                                      self.settings,
                                      self.tr("Open file"),
                                      self.tr("GeoTIFF (*.tif *.tiff *.TIF *.TIFF)")
                                     )
  ...
  def logMessage2(self, message):
    self.txtMessages.append("%s" % (message))
  ...
  def calcChangeMapColorRamp(self, layer, analyst, validationMode, usePercistentClass):
    l = getLayerByName(self.leInitRasterName.text())
