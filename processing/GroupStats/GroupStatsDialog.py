...
from processing.GroupStats.ui_groupstats import Ui_GroupStatsDialog
...
class GroupStatsDialog(QMainWindow):
    def __init__(self, parent=None):
        ...
        #print 'GroupStatsDialog '+str(self.obliczenia.lista[0])
        ...
        QObject.connect(self.ui.wyczysc,SIGNAL("clicked()"), self.wyczyscWybor)
        QObject.connect(self.ui.filtrButton,SIGNAL("clicked()"), self.ustawFiltr)
        QObject.connect(self.ui.warstwa, SIGNAL("currentIndexChanged (int)"), self.wyborWarstwy)   # Sygnał wyboru warstwy
        ...
        QObject.connect(self.tm2, SIGNAL("rowsInserted ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        QObject.connect(self.tm3, SIGNAL("rowsInserted ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        QObject.connect(self.tm4, SIGNAL("rowsInserted ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        QObject.connect(self.tm2, SIGNAL("rowsRemoved ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        QObject.connect(self.tm3, SIGNAL("rowsRemoved ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        QObject.connect(self.tm4, SIGNAL("rowsRemoved ( QModelIndex, int, int )"), self.blokujObliczenia)   # Sygnał wyboru warstwy
        
        QObject.connect(self.ui.actionKopiuj, SIGNAL("triggered ( bool )"), self.kopiowanie)   # Sygnał wyboru warstwy
        QObject.connect(self.ui.actionKopiujZaznaczone, SIGNAL("triggered ( bool )"), self.kopiowanieZaznaczonych)   # Sygnał wyboru warstwy
        QObject.connect(self.ui.actionZapiszCSV, SIGNAL("triggered ( bool )"), self.eksportCSV)   # Sygnał wyboru warstwy
        QObject.connect(self.ui.actionZapiszCSVZaznaczone, SIGNAL("triggered ( bool )"), self.eksportCSVZaznaczonych)   # Sygnał wyboru warstwy    
        QObject.connect(self.ui.actionPokazPanel, SIGNAL("triggered ( bool )"), self.pokazPanelSterowania)   # Sygnał wyboru warstwy
        QObject.connect(self.ui.actionPokazNaMapie, SIGNAL("triggered ( bool )"), self.pokazNaMapie)   # Sygnał wyboru warstwy
        QObject.connect(self.ui.actionTutorial, SIGNAL("triggered ( bool )"), self.pokazTutorial)   # Sygnał wyboru warstwy
        
        QObject.connect(self.ui.wyniki.verticalHeader(), SIGNAL("sortIndicatorChanged ( int, Qt::SortOrder)"), self.sortRows)   # Sygnał wyboru warstwy
    ...
    def pokazWynik(self):               #gotowe
        u"Wykonuje obliczenia i wysyła je do wyswietlenia"
        ...
        if wartosc[0]=='geometria':                                                         # Ustawienie funkcji obliczającej w zależności od typu wybranej wartości
            ...
            wartoscFunkcja = lambda obiekt: None if obiekt.attribute(wartosc[1])==None else obiekt.attribute(wartosc[1])#.toString()    # atrybut tekstowy       API 2.x
        elif wartosc[0]=='atrybutyLicz':
            wartoscFunkcja = lambda obiekt: None if obiekt.attribute(wartosc[1])==None else float(obiekt.attribute(wartosc[1]))         #.toReal()[0]   # atrybut liczbowy (toReal daje wynik (real, True/False))    API 2.x
            
        indeks = self.ui.warstwa.currentIndex()                                             # Pobranie wybranej warstwy
        idWarstwy = self.ui.warstwa.itemData(indeks)
        warstwa = QgsMapLayerRegistry.instance().mapLayer(idWarstwy)#.toString())
        ...
        atrybutyWszy = provider.attributeIndexes()                                          # Wybranie wszystkich obiektów z warstwy
        
        if hasattr(provider, "select"):                                                 # ta część powoduje działanie wtyczki w wersjach poniżej i powyżej QGIS 2.0
            provider.select(atrybutyWszy)                                               # 1.x
            iterator = provider
        else:
            atrybutyWszy=provider.getFeatures()                                         # 2.x
            iterator = atrybutyWszy
            ...
            if tylkoZaznaczone==False or (tylkoZaznaczone and (f.id() in zaznaczoneObiekty)):
                ...
                for k in wybraneKolumny:                                                        # dla każdej wybranej kolumny sprawdzamy typ kolumny
                    if k[0]=='geometria':                                                       # i tworzymy klucz kolumny
                        ...
                        klucz_kol.append(nowyKluczKolumny)#.toString())                    # 2.x szuka po nazwie
                for k in wybraneWiersze:                                                        # dla każdego wybranego wiersza sprawdzmy typ wiersza
                    if k[0]=='geometria':                                                       # i tworzymy klucz wiersza
                        ...
                        klucz_wie.append(nowyKluczWiersza)#.toString())                      # 2.x szuka po nazwie
        ...
        atr =provider.fields()                                                  # ta część powoduje działanie wtyczki w wersjach poniżej i powyżej QGIS 2.0
        if str(type(provider.fields())) != "<type 'dict'>":
            atr = {}
    ...
    def ustawWarstwy (self, warstwy):   #gotowe
        u"Dodaje dostępne wartwy do listy wyboru w oknie"
    ...
    def wyborWarstwy(self, indeks):     #gotowe
        u"Uruchamiane po wybraniu warstwy z listy. Ustawia nową listę pól do wyboru i kasuje okna z już wybranymi polami"
            
        idW = self.ui.warstwa.itemData(indeks)                          # Pobranie ID wybranej warstwy
        warstwa = QgsMapLayerRegistry.instance().mapLayer(idW)#.toString())    
        provider = warstwa.dataProvider()
        atrybuty = provider.fields()                                        # Pobranie atrybutów wybranej warstwy
        
        if str(type(atrybuty)) != "<type 'dict'>":                          # ta część powoduje działanie wtyczki w wersjach poniżej i powyżej QGIS 2.0
            atr = {}
            for i in range(atrybuty.count()):
                atr[i] = atrybuty.at(i)
            atrybuty = atr
            
        slownikPol = {}
            
        if warstwa.hasGeometryType()==False or warstwa.geometryType()==0:                               # point
            slownikPol['geometria'] =  []
        elif warstwa.geometryType()==1:                             # line
            slownikPol ['geometria'] = [(QCoreApplication.translate('GroupStats','Length'), 1)]
        elif warstwa.geometryType()==2:                             # polygon
        ...
        for id, atrybut in atrybuty.iteritems():                            
            if atrybut.typeName().upper() in ('REAL', 'INTEGER', 'INT4', 'INT8', 'FLOAT4'):
                slownikPol['atrybutyLicz'].append((atrybut.name(),id))
            else:
                slownikPol['atrybutyTxt'].append((atrybut.name(),id))
            
        slownikPol['obliczenia']=[]
        obl = self.obliczenia.lista
        for c,b in obl.iteritems():
        ...
        for i in klucze:
            j = slownikPol[i] 
            j.sort(lambda x,y: cmp(x[0].lower(),y[0].lower()))
            wiersze=[]
            for k, l in j:
        ...
        wiersze.sort(lambda x,y: cmp(x[1].lower(),y[1].lower()))
        self.tm1.insertRows( 0, len(wiersze), QModelIndex(), wiersze)    
    ...
    def wyczyscWybor(self):             # gotowe
        u" Czyści okna z wybranymi wierszami, kolumnami i wartościami"
    ...
    def ustawFiltr(self):               # gotowe 2
        indeks = self.ui.warstwa.currentIndex()                                             # Pobranie wybranej warstwy
        idWarstwy = self.ui.warstwa.itemData(indeks)
        warstwa = QgsMapLayerRegistry.instance().mapLayer(str(idWarstwy))
    ...
    def kopiowanie (self):
        u"Kopiowanie wszystkich danych do schowka"
    ...
    def kopiowanieZaznaczonych (self):
        u"Kopiowanie zaznaczonych danych do schowka"
    ...
    def zapiszDaneWPliku (self, dane):
        ...
        plik = open(nazwaPliku, 'wb')                                           # Otwarcie pliku do zapisu
        plikCSV = csv.writer( plik, delimiter=';' )
        for i in dane:                                                          # Kopiowanie danych z tabeli                        
            plikCSV.writerow([x.encode('utf-8') for x in i])
        plik.close()
    ...
    def pokazNaMapie(self):             # zmienic zeby nie dublowac indeksow z komorek
        ...
        if len(listaId)>0:                                                                  # Jeśli wybrano jakieś obiekty:
            self.tm5.warstwa.setSelectedFeatures(listaId)                                   #   zaznaczenie ich na mapie
...
class ModelList(QAbstractListModel):
    ...
    def insertRows(self, wiersz, liczba, indeks, dane):
        ...
        for n in range(liczba): self.dane.insert(wiersz+n, dane[n])
        self.endInsertRows()
        return True
    ...
    def mimeData(self, indeksy, typMime='application/x-groupstats-polaL'):
        ...
        for indeks in indeksy:
            ...
            strumien.writeBytes(self.dane[wiersz][0])
            strumien.writeBytes(self.dane[wiersz][1].encode('utf-8'))           #--------------------------------------------------------
...
class ModelWyniki(QAbstractTableModel):     # gotowe
    ...
    def sort(self, kolumna, tryb):
        ...
        tmp.sort(lambda x,y: cmp(x[1],y[1]))                                    # sortowanie rosnąco
    ...
    def sortRows(self, wiersz, tryb):   # 2.0 ready
        u"""
...
class OknoWyniki(QTableView):     # 2.0 ready
    u"""
    ...
    def __init__(self, parent=None):
        ...
        QObject.connect(self, SIGNAL("clicked (QModelIndex)"), self.zaznaczWszystko)
...
class Obliczenia(QObject):                   # gotowe
    ...
    def __init__(self, parent):                                                                            # Lista z ID, nazwą i funkcją obliczająca
        ...
        #print 'Obliczenia '+str(self.lista[0])
