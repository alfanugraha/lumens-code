...
#LU1
rat_lu1<-ratify(landuse1)
colnames(lookup_lc)<-c("ID", "CLASS", "CARBON")
lu1_lookup<-merge(levels(rat_lu1),lookup_lc, by="ID")
lu1_lookup$Rowid<-NULL
lu1_lookup$COUNT<-NULL
lu1_lookup$CARBON<-NULL
...
data_zone<-zone_lookup
data_zone$Z_CODE<-toupper(abbreviate(data_zone$ZONE))
for(i in 1:lg){
...
data_zone[,4:7]<-round(data_zone[,4:7],digits=3)

#calculate largest sorce of emission
...
e.m.z<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
...
e.m<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
for (i in 1:nrow(e.m)){
for (j in 1:ncol(e.m)){
...
s.m.z<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
...
s.m<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
for (i in 1:nrow(s.m)){
for (j in 1:ncol(s.m)){
...
n.m.z<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
...
#Net emission matrix
n.m<-matrix(0, nrow=length(area_lc1$ID), ncol=length(area_lc2$ID))
for (i in 1:nrow(n.m)){
for (j in 1:ncol(n.m)){
