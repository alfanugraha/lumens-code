...
from processing.molusce.algorithms.models.mce.mce import MCE

from processing.molusce.spinboxdelegate import SpinBoxDelegate

from processing.molusce.ui.ui_multicriteriaevaluationwidgetbase import Ui_Widget
    ...
    for k, v in self.inputs["factors"].iteritems():
      for b in xrange(v.getBandsCount()):
        if v.getBandsCount()>1:
          name = self.tr("%s (band %s)") % (getLayerById(k).name(), unicode(b+1))
        else:
