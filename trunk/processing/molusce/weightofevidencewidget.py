...
from processing.molusce.algorithms.models.woe.manager import WoeManager, WoeManagerError
from processing.molusce.algorithms import dataprovider

from processing.molusce.spinboxdelegate import SpinBoxDelegate

from processing.molusce.ui.ui_weightofevidencewidgetbase import Ui_Widget
...
class WeightOfEvidenceWidget(QWidget, Ui_Widget):
  ...
  def manageGui(self):
    ...
    if not checkChangeMap(self.inputs):
      QMessageBox.warning(self.plugin,
                          self.tr("Missed input data"),
    ...
    self.delegate = SpinBoxDelegate(self.tblReclass.model(), minRange=2, maxRange=dataprovider.MAX_CATEGORIES)
      ...
      for b in xrange(1, v.getBandsCount()+1):
        if v.isCountinues(b):
          ...
          if v.getBandsCount()>1:
            name = u"%s (band %s)" % (getLayerById(k).name(), unicode(b))
          else:
