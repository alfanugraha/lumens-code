...
from processing.reclasstable.tableManagerUi import Ui_Dialog
from processing.reclasstable.tableManagerUiRename import Ui_Rename
from processing.reclasstable.tableManagerUiClone import Ui_Clone
from processing.reclasstable.tableManagerUiInsert import Ui_Insert
import os,sys
...
class ReclassTable(QDialog, Ui_Dialog):

  def __init__(self, iface):
    QDialog.__init__(self)
    self.iface = iface
    self.setupUi(self)
    #/home/alfa/Desktop/LUMENS/Sample_data/Carbon_stock.csv","jambi_district
    self.layer = QgsVectorLayer(os.path.join(os.path.dirname(__file__), "reclass_template.shp"),"reclass_template","ogr")
    ...
    QObject.connect(self.butInsRow, SIGNAL('clicked()'), self.doInsertRow)
    QObject.connect(self.tabWidget, SIGNAL('currentChanged (int)'), self.drawDataTable)
#     QObject.connect(self.butReconcile, SIGNAL('clicked()'), self.doReconcile2) #Added by Alfa #alternative reconcile
    QObject.connect(self.dataTable, SIGNAL('cellDoubleClicked(int, int)'), self.doReconcile)
  ...
  def drawFieldsTable(self): # Draws the fields table on startup and redraws it when changed
    fields = self.fields
#     self.fieldComboBox.clear() # Added by Alfa: alternative reconcile
    ...
    for i in range(len(fields)):
      ...
      #set field combobox
#       self.fieldComboBox.addItem(fields[i].name()) #alternative reconcile
...
#     table.item(1, 1).isChanged
#     table.cellClicked()
#     table.insertRow()

  def doReconcile(self):
    self.setChanged()
#     a = self.dataTable.item(1,1).text()
#     b = self.data[1][1]
#     QMessageBox.warning(self.iface.mainWindow(), QCoreApplication.translate('TableManager', 'Table Manager'), QCoreApplication.translate('TableManager', 'Please select a ('+ str(b) + ') vector layer'))


#   Added by Alfa #alternative reconcile
#   def doReconcile2(self):
#     inputLayer = self.layer # "/home/alfa/Documents/Jambi/tanjabar_subdistrict.shp"
#     fieldName = self.fieldComboBox.currentText() 
#     outputs = processing.runalg("qgis:reconcile",inputLayer,fieldName, None)
#     output = outputs["OUTPUT"]
#     dataobjects.load(output, "Reconciled")
#     self.butReconcile.setEnabled(False) 
  ...
  def doMoveUp(self): # Called when appropriate button was pressed
    ...
    self.drawFieldsTable() # Added by Alfa
    self.setChanged()
  ...
  def doInsert(self): # Called when appropriate button was pressed
    ...
    if dlg.exec_() == QDialog.Accepted:
      (aName, aType, aPos) = dlg.result() #nama, tipe, posisi
      if aType == 0:
        aLength = 10
        aPrec = 0
  ...
  def doInsertRow(self):
    ...
    itemCell1 = QTableWidgetItem(str(row+1))
    itemCell2 = QTableWidgetItem('NULL')
    self.dataTable.insertRow(row)
    self.dataTable.setItem(row, 0, itemCell1)
    self.dataTable.setItem(row, 1, itemCell2)
...
#     self.butReconcile.setEnabled(True) #Added by Alfa #alternative reconcile
  ...
  def writeToFile(self, filePath, encoding, driverName = 'ESRI Shapefile'): # write data to a shapefile
    ...
    for feat in self.provider.getFeatures():
        ...
        for i in range(len(self.fields)):
            try:
                #default -> self.data[i][n]
                #transpose table -> self.dataTable.item(i,n).text()
                attrs += [self.dataTable.item(n,i).text()] 
            except:
                attrs += ['']
